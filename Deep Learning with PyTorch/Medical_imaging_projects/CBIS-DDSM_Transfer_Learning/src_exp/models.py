...
class CBISDDSMPatchClassifierVGG(pl.LightningModule):
    ...
    def configure_optimizers(self, mode=None):
        if mode is None:
            # initially set all layer learning rate to lr
            optimizer = Adam(filter(lambda p: p.requires_grad,
                                    self.model.parameters()), lr= self.lr['init'])
        else:
            # only update optimizer for a specific layer if mod eis provided
            optimizer = Adam(filter(lambda p: p.requires_grad,
...
class CBISDDSMPatchClassifierResNet(pl.LightningModule):
    ...
    def configure_optimizers(self, mode=None):
        if mode is None:
            optimizer = Adam(filter(lambda p: p.requires_grad,
                                    self.model.parameters()), lr=0.0009)
        else:
            optimizer = Adam(filter(lambda p: p.requires_grad,
                                    self.model.parameters()), lr=self.lr[mode])
        return optimizer
    ...
    # unfreeze last 'n' fully connected layers
    def unfreeze_last_n_basic_blocks(self, n):

        # if n == -1 don't unfreeze any layers
        if n == -1:
            return 0

        n = n * 4  # since weights and bias are included as separate
        total_layers = len(list(self.model.layer4.parameters()))
        ...
        # if n == 0 unfreeze all layers
            ...
            for i, param in enumerate(self.model.layer4.parameters()):
                if i >= total_layers - n:
                    param.requires_grad = True
                else:
                    pass
    ...
    def set_transfer_learning_params(self, unfreeze_n_fc, unfreeze_n_conv, optim_fc=False, optim_conv=False):
        ...
        if optim_fc:
            self.configure_optimizers('fc')
        self.unfreeze_last_n_basic_blocks(unfreeze_n_conv)
        if optim_conv:
            self.configure_optimizers('conv')
